...
wind.title("Ping Pong By Faraag") #Set the title of the window.
...
wind.onkeypress(madrab1_up, "w") #when prssing w the function madrab1_up is invoked
...
while True:
    ...
    #move the ball
    ball.setx(ball.xcor() + ball.dx)#ball starts at 0 and everytime loop run-->+.3 xaxis
    ball.sety(ball.ycor() + ball.dy)#ball starts at 0 and everytime loop run-->+.3 xaxis
    ...
    if ball.ycor() > 290: #if ball is at top border 
        ...
        ball.dy *= -1 #reverse direction, making +.3 -----> -.3
    
    elif ball.ycor() < -290: #if ball is bottom border 
         ball.sety(-290)
         ball.dy *= -1
    
    elif ball.xcor() >390: #if ball is at right border
    ...
    elif ball.xcor() <-390:  # if ball is at left border
         ball.goto(0, 0)
         ball.dx *= -1
         score2 +=1
         score.clear()
